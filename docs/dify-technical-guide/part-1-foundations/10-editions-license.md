...
> **DOC-G3-L:** Review package ready; deployment-specific Legal/Security/Procurement determination pending
...
Đây là bản tóm tắt để định tuyến review, không thay exact license text hoặc tư vấn pháp lý. Gói `DOC-G3-L` phải chuyển nguyên văn immutable source [S-004] cùng bản tóm tắt này cho reviewer.

Brief đầu vào dùng cụm “source-available, không phải open-source thuần”. Tài liệu này **không nâng cụm đó thành fact đã xác minh** vì file `LICENSE` tự gọi cơ chế là “modified Apache License 2.0” và việc gán taxonomy pháp lý rộng hơn thuộc Legal. Cách ghi chuẩn trong core guide là: **“Dify Community `1.15.0` dùng modified Apache License 2.0 với điều kiện bổ sung; applicability và classification cần Legal xác nhận theo mô hình sử dụng.”** Nếu tổ chức muốn dùng nhãn “source-available”, Legal phải ghi definition, phạm vi và approval evidence trong deployment profile. [S-004]
