...
> **Trạng thái xác minh:** `Design reviewed` qua cross-review nội bộ; scenario walkthrough minh họa đã hoàn tất, specialist review và POC theo tổ chức vẫn `RUNTIME-PENDING`
>
...
### D17 — Stage-gate từ ý tưởng đến quyết định POC/pilot

```mermaid
flowchart LR
    Idea["POC-G0: Vấn đề và đầu mối chịu trách nhiệm"] --> Scope["POC-G1: Phạm vi, mẫu use case và chỉ số"]
    Scope --> Ready["POC-G2: Sẵn sàng dữ liệu, model, tool và môi trường"]
    Ready --> Build["POC-G3: Xây dựng lát cắt nhỏ nhất"]
    Build --> Eval["POC-G4: Đánh giá chất lượng, an toàn, hiệu năng và chi phí"]
    Eval --> Security["POC-G5: Kiểm thử bảo mật và tình huống lỗi"]
    Security --> Gate{"POC đạt mọi điều kiện bắt buộc?"}

    Gate -->|"Không, giả thuyết không còn giá trị"| Stop["STOP + lưu bằng chứng và bài học"]
    Gate -->|"Không, có thể khắc phục"| Revise["REVISE phạm vi, cấu hình hoặc dữ liệu"]
    Revise --> Ready

    Gate -->|"Có"| Demo["POC-G6: Demo có kiểm soát"]
    Demo --> Pilot["POC-G7: Pilot giới hạn người dùng"]
    Pilot --> Decision{"POC-G8: GO, REVISE, HOLD hay STOP?"}
    Decision -->|"GO"| Plan["Kế hoạch production + đầu mối + ngân sách"]
    Decision -->|"REVISE"| Ready
    Decision -->|"HOLD / STOP"| Close["Điều kiện xem lại hoặc đóng POC"]
```

Không bỏ qua `POC-G5` chỉ vì demo thành công; không mở pilot tại `POC-G7` trước khi demo ở `POC-G6` công khai giới hạn; không chọn `GO` tại `POC-G8` khi điều kiện vận hành, backup/DR, đầu mối hoặc ngân sách còn thiếu.

## Hướng dẫn hoặc ví dụ triển khai

### POC-G0 — Tiếp nhận và xác định vấn đề
...
### POC-G1 — Phạm vi và tiêu chí thành công
...
### POC-G2 — Sẵn sàng về dữ liệu, model, tool và môi trường
...
### POC-G3 — Xây dựng lát cắt nhỏ nhất
...
### POC-G5 — Kiểm thử bảo mật và tình huống lỗi
...
### POC-G7 — Pilot giới hạn người dùng
...
### POC-G8 — Quyết định GO, REVISE, HOLD hoặc STOP
...
|---|---|---|---|
| “POC thành công” chỉ vì demo chạy | Không có baseline/set/threshold | Scope theo feature | Quay lại `POC-G0`/`POC-G1`; định nghĩa hypothesis và hard gates. |
