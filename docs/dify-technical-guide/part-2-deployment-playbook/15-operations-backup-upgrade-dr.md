...
> **Trạng thái xác minh:** `Official-source verified` + `Config validated` + `Design reviewed`; mọi phép đo, restore drill, upgrade rehearsal và DR/failover đều `RUNTIME-PENDING`  
...
8. chỉ sau đó start API/web/worker/plugin theo rollout plan;
9. chạy release-specific one-shot/backfill;
